import React from 'react'
import styled from "./Footer.module.css"
const Footer = () => {
    return (
        <div>
        <div className={styled.footer}>
            <div className={styled.main_row ,styled.flex}>
                <div className={styled.fotr_col_2}>
                    <h3>Information</h3>
                    <a href="#"><td>About us</td></a><br/>
                    <a href="#"><td>Terms and condition</td></a><br/>
                    <a href="#"><td>Careers with us</td></a><br/>
                    <a href="#"><td>Sitemap</td></a><br/>
                    <a href="#"><td>Contact us</td></a><br/>
                    <a href="#"><td>FAQs</td></a><br/>
                    <a href="#"><td>Summons/Notices</td></a><br/>
                    <a href="#"><td>Grievances</td></a><br/>
                    <a href="#"><td>Fraud Alter</td></a><br/>
                    <a href="#"><td>Trust and sefety</td></a><br/>
                    <br/>
                    <div className={styled.fotr_col_1}>
                     <h2>Naukri on Mobile</h2>
                    <tbody className={styled.share_icon}>

                        <td>
                            <img style={{width:"150px",height:"35px",marginTop:"10px"}} src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQBKp71Uq-A4LYdwRLCd8Qnl4yv_pQ7DjvFnA&usqp=CAU" alt="google"/>
                        </td><br/>
                        <td><img style={{width:"150px",height:"35px",marginTop:"20px"}}  src="data:image/png;base64," alt="apple"/></td>
                        
                    </tbody>
                    </div>
                </div>
                <div className={styled.fotr_col_3}>
                    <h3>Jobseekers</h3>
                    <a href=""><td>Register Now</td></a><br/>
                    <a href=""><td>Job search</td></a><br/>
                    <a href=""><td>Login</td></a><br/>
                    <a href=""><td>Create job Alert</td></a><br/>
                    <a href=""><td>Report a Problem</td></a><br/>
                    <a href=""><td>Naukri Blogs</td></a><br/>
                    <a href=""><td>Mobile Site</td></a><br/>
                    <br/>
                    <h3>Browse Jobs</h3>
                    <a href=""><td>Browse all job</td></a><br/>
                    <a href=""><td>Premium MBA job</td></a><br/>
                    <a href=""><td>Premium Engineering job</td></a><br/>
                    <a href=""><td>Govt. job</td></a><br/>
                    <a href=""><td>Internation job</td></a><br/>
                    <a href=""><td>Jobs in Gulf</td></a><br/>
                    <a href=""><td>Jobs by Company</td></a><br/>
                    <a href=""><td>Jobs by Category</td></a><br/>
                    <a href=""><td>Jobs by destination</td></a><br/>
                    <a href=""><td>Jobs by location</td></a><br/>
                    <a href=""><td>Jobs by skill</td></a><br/>
                </div>
                <div className={styled.fotr_col_4}>
                    <h3>Jobseeker Services</h3>
                    <a href=""><td>Priority Applicant</td></a><br/>
                    <a href=""><td>Resume display</td></a><br/>
                    <a href=""><td>Resume writting</td></a><br/>
                    <a href=""><td>Job4u</td></a><br/>
                    <a href=""><td>Recruiter Connection</td></a><br/>
                    <br/>
                    <h3>Naukri Learning</h3>
                    <a href=""><td>Softer and technology</td></a><br/>
                    <a href=""><td>AI AND DS</td></a><br/>
                    <a href=""><td>Management</td></a><br/>
                    <a href=""><td>Finance</td></a><br/>
                    <a href=""><td>Creativity and Design</td></a><br/>
                    <a href=""><td>Emerging Technology</td></a><br/>
                    <a href=""><td>Engineering non CS</td></a><br/>
                    <a href=""><td>Healthcare</td></a><br/>
                    <a href=""><td>Enery and Enviroment</td></a><br/>
                    <a href=""><td>Social Science</td></a><br/>
                    <a href=""><td>Personal Growth</td></a><br/>
                </div>
                <div className={styled.fotr_col_5}>
                    <h3>Employers</h3>
                    <a href=""><td>Job posting</td></a><br/>
                    <a href=""><td>Resume database access</td></a><br/>
                    <a href=""><td>Recruiter Login</td></a><br/>
                    <a href=""><td>Naurkri RMS</td></a><br/>
                    <a href=""><td>By resume package online</td></a><br/>
                    <a href=""><td>Transition services</td></a><br/>
                    <a href=""><td>report a problem</td></a><br/>
                    <a href=""><td>Recruite from USA ,call</td></a><br/>
                    <a href=""><td>Toll Free # 1866-557-3340</td></a><br/>
                    <br/>
                    <h3>Ambition Box</h3>
                    <a href=""><td>Inverview Question</td></a><br/>
                    <a href=""><td>About Companies</td></a><br/>
                    <a href=""><td>Share interview advice</td></a><br/>
                    <a href=""><td>Write company review</td></a><br/>
                    <a href=""><td>Companies reviews</td></a><br/>
                    <a href=""><td>Companies salaries</td></a><br/>
                    <a href=""><td>Follow us</td></a><br/>
                    <div className={styled.fotr_col_1}>
                    <tbody className={styled.share_icon}>
                        <td>
                            <img src="https://image.flaticon.com/icons/png/128/1384/1384053.png" alt="facebook"/>
                        </td>
                        <td><img src="https://image.flaticon.com/icons/png/128/1409/1409937.png" alt="twitter"/></td>
                        <td><img src="https://image.flaticon.com/icons/png/128/3256/3256016.png" alt="linkedin"/></td>
                    </tbody>
                    </div>
             
                </div>
         

            </div>

        </div>
        </div>
  )
}

export default Footer
